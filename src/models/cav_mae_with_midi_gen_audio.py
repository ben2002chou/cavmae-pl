...
class CAVMAE(nn.Module):
    ...
    def forward_encoder(
        ...
        a2 = a2 + self.modality_a  # Assuming you use the same modality parameter
    ...
    def forward(
        ...
        if mae_loss_weight != 0:
            ...
            loss_mae_a1 = self.forward_mae_loss(audio1, pred_a1, mask_a1, "a")
            loss_mae_a2 = self.forward_mae_loss(audio2, pred_a2, mask_a2, "a")
        ...
        if contrast_loss_weight != 0:
            ...
            loss_c_a1, c_acc_a1 = self.forward_contrastive(
                latent_c_a1.mean(dim=1), latent_c_v.mean(dim=1)
            )
            loss_c_a2, c_acc_a2 = self.forward_contrastive(
                latent_c_a2.mean(dim=1), latent_c_v.mean(dim=1)
            )

            # Combining contrastive losses from both audio streams
    ...
    def forward_inpaint(
        ...
        (
            ...
            audio, imgs, mask_ratio_a1, mask_ratio_a2, mask_ratio_v, mask_mode=mask_mode
        )
        pred_a1, pred_a2, pred_v = self.forward_decoder(
            latent,
            mask_a1,
        ...
        loss_pixel_a1 = self.forward_mae_loss(audio, pred_a1, mask_a1, "a1")
        loss_pixel_a2 = self.forward_mae_loss(audio, pred_a2, mask_a2, "a2")
